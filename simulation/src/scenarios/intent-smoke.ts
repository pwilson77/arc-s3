...
async function main(): Promise<void> {
  ...
  const firewall = new Contract(config.S3_INTENT_FIREWALL, intentFirewallAbi, ctx.provider);
  ...
  const [alphaRegistered, betaRegistered, courthouseWhitelisted] = await Promise.all([
  ...
  const createData = escrowAlpha.interface.encodeFunctionData("forwardCreateTask", [
    ctx.alpha.address,
    ctx.beta.address,
    config.DEFAULT_TASK_PAYMENT,
    config.DEFAULT_BOND_AMOUNT,
  ...
  const createdArgs = parseEventArg(createReceipt.logs, escrowAlpha, "TaskCreated");
  const taskId = String(createdArgs.taskId);
  console.log(`Intent createTask passed taskId=${taskId}`);

  const acceptData = escrowBeta.interface.encodeFunctionData("forwardAcceptTask", [
    ctx.beta.address,
    taskId,
  ]);
  ...
  console.log(`Intent submitTaskResult passed taskId=${taskId} trace=${traceHash}`);
