...
import { buildSocialAllocations } from "@/lib/social-intel";
import { readLatestRfb6Run, readRfb6Runs } from "@/lib/rfb6-agent";
...
export default async function DashboardPage() {
  ...
  const allocations = latestRfb6Run
    ? latestRfb6Run.workers
    : buildSocialAllocations(ranked);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-10">
        <Stat label="registered agents" value={String(REGISTERED_AGENTS.length)} />
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {REGISTERED_AGENTS.map((agent) => {
            ...
            if (isRfb6) {
              statusLine = hasRfb6Runs ? "trace stream active" : "no runs yet";
            } else if (latestMetrics) {
              statusLine = `settled ${latestMetrics.aggregate.totalSettled} · slash ${
                (latestMetrics.aggregate.slashRateBps / 100).toFixed(2)
              }%`;
            }
            ...
            return (
              ...
              >
                ...
                <div className="text-neutral-100 font-mono mb-2">{agent.id}</div>
                <div className="text-[11px] text-neutral-500 font-mono mb-2 break-all">{agent.erc8004Id}</div>
                <div className="text-xs text-neutral-400 leading-relaxed mb-3">{agent.summary}</div>
                <div className="text-[11px] text-neutral-500 leading-relaxed mb-3">{agent.responsibility}</div>
                <div
                  className={
      ...
      <section className="mb-12">
        ...
        <div className="text-xs text-neutral-500 mb-3">
          source: {latestRfb6Run ? "standalone rfb6 process" : "dashboard local fallback"}
        </div>
      ...
      <section className="mb-12">
        ...
        {recentRfb6Runs.length === 0 ? (
          <div className="text-xs text-neutral-500">no standalone rfb6 runs yet.</div>
        ) : (
          <table className="w-full text-xs">
            ...
            <tbody>
              {[...recentRfb6Runs].reverse().slice(0, 10).map((run) => (
      ...
      <section className="mb-12">
        <h2 className="text-[11px] text-neutral-500 mb-4 uppercase tracking-wider font-mono">recent settlements</h2>
        <table className="w-full text-xs">
          <thead>
            <tr className="text-left text-[11px] text-neutral-500 uppercase tracking-wider font-mono border-b border-neutral-800">
              <th className="py-2 pr-4 font-normal">time</th>
          ...
          <tbody>
            {recentEvents.map((e) => (
              ...
              >
                ...
                <td className="py-2 pr-4 text-neutral-200 font-mono">{e.worker}</td>
      ...
      <section>
        <h2 className="text-[11px] text-neutral-500 mb-4 uppercase tracking-wider font-mono">integrity incidents</h2>
